...
export default function NSNDetailsUpdateForm(props) {
  const {
    nsn: nsnProp,
    nSNDetails,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = nSNDetailsRecord
      ? { ...initialValues, ...nSNDetailsRecord }
      : initialValues;
  ...
  const [nSNDetailsRecord, setNSNDetailsRecord] = React.useState(nSNDetails);
  React.useEffect(() => {
    const queryData = async () => {
      const record = nsnProp
        ? await DataStore.query(NSNDetails, nsnProp)
        : nSNDetails;
      setNSNDetailsRecord(record);
    };
    queryData();
  }, [nsnProp, nSNDetails]);
  React.useEffect(resetStateValues, [nSNDetailsRecord]);
  const validations = {
    nsn: [{ type: "Required" }],
    niin: [],
    itemName: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            NSNDetails.copyOf(nSNDetailsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "NSNDetailsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nsn"
